...
bool CGuildMarkManager::GetMarkImageFilename(DWORD imgIdx, std::string & path) const
{
	...
	char buf[64];
	snprintf(buf, sizeof(buf), "data/symbol_guild/%s_%lu.tga", m_pathPrefix.c_str(), imgIdx);
	path = buf;
	return true;
}
...
{
	char buf[64];
	snprintf(buf, sizeof(buf), "data/symbol_guild/%s_index", m_pathPrefix.c_str());
	FILE * fp = fopen(buf, "r");
...
bool CGuildMarkManager::SaveMarkIndex()
{
	...
	fclose(fp);
	auto result = sys_log(0, "MarkManager::SaveMarkData: index count %d", m_mapGID_MarkID.size());
	return true;
}
