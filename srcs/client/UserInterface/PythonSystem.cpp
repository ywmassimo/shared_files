...
bool CPythonSystem::LoadConfig()
{
	...
	if (NULL == (fp = fopen("data/settings_game/metin2.cfg", "rt")))
		return false;
...
bool CPythonSystem::SaveConfig()
{
	...
	if (NULL == (fp = fopen("data/settings_game/metin2.cfg", "wt")))
		return false;
...
{
	FILE* f = fopen("data/history_systems/chat_filter.bin", "rb");
	if (!f)
		return;
...
{
	FILE* f = fopen("data/history_systems/chat_filter.bin", "wb");
	if (!f)
		return;
