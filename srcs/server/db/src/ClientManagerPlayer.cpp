...
void CClientManager::RESULT_COMPOSITE_PLAYER(CPeer * peer, SQLMsg * pMsg, DWORD dwQID)
{
	...
	switch (dwQID)
	{
		...
		case QID_QUEST:
		{
			...
			//@massimo
			sys_log(0, "info of pLoginData1 before call ItemAward function %p, login: %s", pLoginData1, pLoginData1->GetAccountRef().login);
			// sys_log(0, "info of pLoginData1 before call ItemAward function %p", pLoginData1);
