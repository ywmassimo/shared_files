...
LPITEM CHARACTER::GetWear(BYTE bCell) const
{
	...
	if (bCell >= static_cast<int>(WEAR_MAX_NUM) +
				 static_cast<int>(DRAGON_SOUL_DECK_MAX_NUM) *
				 static_cast<int>(DS_SLOT_MAX))
	{
					...
					case USE_TALISMAN:
						{
							...
							else if (item->GetValue(0) == MEMORY_PORTAL)
							{
								if (item->GetSocket(0) == 0)
								{
									if (GetDungeon())
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("던전 안에서는 %s%s 사용할 수 없습니다."), item->GetClientName(), "");
										return false;
									}
	...
	if (item->DistanceValid(this))
	{
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
				...
				if (finalItem)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 획득: %s"), finalItem->GetClientName());
				...
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 획득: %s"), item->GetClientName());
		...
		else if (!IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_DROP) && GetParty())
		{
			...
			{
				owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 획득: %s 님으로부터 %s"), GetName(), item->GetClientName());
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 전달: %s 님에게 %s"), owner->GetName(), item->GetClientName());
			}
...
LPITEM CHARACTER::AutoGiveItem(DWORD dwItemVnum, BYTE bCount, int iRarePct, bool bMsg)
{
	...
	{
		if (bMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 획득: %s"), foundItem->GetClientName());
		return foundItem;
	}
